  ...
  const fetchArticles = async () => {
    ...
    try {
      ...
      const [devtoRes, mediumRes] = await Promise.all([
        fetchWithRetry(
          `https://dev.to/api/articles?username=${developerInfo.social.devto}`,
        ),
        fetchWithRetry(
          `https://api.rss2json.com/v1/api.json?rss_url=https://medium.com/feed/${developerInfo.social.medium}`,
        ),
      ]);
      ...
      const transformedMediumArticles = mediumRes.items.map(
      ...
      const allArticles = [
        ...transformedDevtoArticles,
        ...transformedMediumArticles,
      ];
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="card animate-pulse backdrop-blur-sm bg-white/50 dark:bg-gray-800/50"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredArticles.length > 0 ? (
              filteredArticles.map((article) => (
                <div
                  key={article.id}
                  className="group bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-lg shadow-sm border border-gray-100/50 dark:border-gray-700/50 hover:shadow-md transition-all duration-300 p-6 hover:bg-white/90 dark:hover:bg-gray-700/90"
                >
                  ...
                  {article.tags && article.tags.length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-4">
                      {article.tags.slice(0, 3).map((tag, index) => (
                        <span
                          key={index}
                          className="px-2 py-1 bg-gray-100/80 dark:bg-gray-700/80 backdrop-blur-sm text-gray-600 dark:text-gray-300 text-xs rounded-full"
                        >
                          {tag}
                        </span>
                      ))}
